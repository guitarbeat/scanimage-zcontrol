classdef UIComponentFactory < handle
    ...
    methods (Static)
        ...
        function [statusText, statusBar] = createStatusBar(parent)
            % Creates a simple status bar
            statusBar = uipanel(parent, ...
                'Position', [0 0 parent.Position(3) 24], ...
                'BorderType', 'none', ...
                'BackgroundColor', [0.9 0.9 0.95]);
            
            statusText = uilabel(statusBar, ...
                'Position', [10 2 parent.Position(3)-20 20], ...
                'Text', 'Ready', ...
                'FontSize', 10);
            
            % Update position on figure resize
            parent.SizeChangedFcn = @(~,~) updateStatusBarSize(parent, statusBar, statusText);
            
            function updateStatusBarSize(fig, bar, text)
                % Update status bar size when figure resizes
                bar.Position = [0 0 fig.Position(3) 24];
                text.Position = [10 2 fig.Position(3)-20 20];
            end
        end
